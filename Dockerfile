...
# ==================== Node.js v24 ====================
# hermes-web-ui 要求 Node >= 24.0.0
RUN ARCH=$(dpkg --print-architecture) \
    && if [ "$ARCH" = "amd64" ]; then NODE_ARCH="x64"; else NODE_ARCH="$ARCH"; fi \
    && echo "Installing Node.js v24.12.0 for ${NODE_ARCH}" \
    && curl -fsSL "https://nodejs.org/dist/v24.12.0/node-v24.12.0-linux-${NODE_ARCH}.tar.gz" \
       -o /tmp/node.tar.gz \
...
# /usr/local/bin 已在默认 PATH 中

# ==================== baoyu-skills 脚本预置 ====================
# 将完整的 baoyu-imagine 脚本预置到 ~/.baoyu-skills/ 目录
# 此目录是 main.ts loadExtendConfig() 的查找路径之一
# 避免依赖 Web UI 技能安装（可能只下载编译产物而丢失 .ts 源文件）
# 上游仓库当前将该技能放在 baoyu-image-gen 目录下；保留兼容分支以便后续重命名不再中断构建
RUN mkdir -p /data/.baoyu-skills/baoyu-imagine && \
    git clone --depth 1 https://github.com/JimLiu/baoyu-skills.git /tmp/baoyu-skills && \
    if [ -d /tmp/baoyu-skills/skills/baoyu-image-gen/scripts ]; then \
        cp -r /tmp/baoyu-skills/skills/baoyu-image-gen/scripts \
              /data/.baoyu-skills/baoyu-imagine/scripts; \
    elif [ -d /tmp/baoyu-skills/skills/baoyu-imagine/scripts ]; then \
        cp -r /tmp/baoyu-skills/skills/baoyu-imagine/scripts \
              /data/.baoyu-skills/baoyu-imagine/scripts; \
    else \
        echo "baoyu-skills scripts directory not found" >&2; \
        exit 1; \
    fi && \
...
# 保留源码到 /opt/hermes-agent，hermes-web-ui 的 agent-bridge 需要 run_agent.py
RUN git clone --depth 1 https://github.com/NousResearch/hermes-agent.git /opt/hermes-agent && \
    pip install --no-cache-dir /opt/hermes-agent[all] && \
...
RUN git clone --depth 1 https://github.com/Brumaire01/hermes-web-ui.git /tmp/hermes-web-ui && \
...
# 创建数据目录
RUN mkdir -p /data/.hermes /app/logs && \
...
RUN set -eux; \
    chmod 755 /usr/local/bin/hermes-agent; \
    ln -sfn /usr/local/bin/hermes-agent /data/.hermes/hermes-agent; \
    chmod +x /opt/hermes-web-ui/dist/server/agent-bridge/hermes_bridge.py
