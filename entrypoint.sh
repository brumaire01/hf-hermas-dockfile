...
# bun 已安装在 /usr/local/bin（全局可访问），/root/.local/bin 用于 wrapper 脚本
export PATH="$PATH:/usr/local/bin:/root/.local/bin"

# hermes-web-ui 的 agent-bridge 需要 hermes-agent 源码路径（含 run_agent.py）
export HERMES_AGENT_ROOT="${HERMES_AGENT_ROOT:-/opt/hermes-agent}"
...
BAOYU_SKILLS_BASE="/data/.baoyu-skills"
...
if [ -f /data/.baoyu-skills/.env ]; then
    set -a
    source /data/.baoyu-skills/.env
    set +a
fi
...
if [ "$API_READY" = true ]; then
    ...
    # 通过 symlink /root/.hermes → /data/.hermes，BFF GatewayManager 可正确读取
else
